...
public class GeneralCommands implements CommandExecutor, TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        while (it.hasNext()) {
            if (!((String)it.next()).startsWith(finalArg)) {
                it.remove();
            }
        }
        ...
        if (args.length < 1 || "help".equalsIgnoreCase(args[0])) {
            ...
            if (args.length > 1) {
                try {
                    ...
                    sender.sendMessage(String.format(this.plugin.getLocale().getString("not-number"), new Object[] { args[1] }));
                    return true;
                }
            ...
            if (args.length < 2) {
                ...
                if (player == null) {
                    sender.sendMessage(String.format(this.plugin.getLocale().getString("player-not-found"), new Object[] { args[1] }));
                    return true;
                }

                this.plugin.getFishShopGUI().openGUI(player);
                sender.sendMessage(String.format(this.plugin.getLocale().getString("forced-player-to-shop"), new Object[] { player.getName() }));
            }
